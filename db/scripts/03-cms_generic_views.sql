...
#User page: Add User, Contact List Combo (Select contact to Create user:)
	CREATE OR REPLACE VIEW `vw_user_create_list` AS 
		SELECT a.id, a.f_name, a.l_name, a.email, a.mobile, b.Name as ctype 
			...
			WHERE a.contacttype_id = b.id AND b.id in (2,3) AND a.id NOT IN (SELECT contact_id FROM tbl_users)  ORDER BY a.id;
...
# View for Users List in Create/Edit User default
	CREATE OR REPLACE VIEW `vw_userlist` AS   
		SELECT a.id, a.user_name, b.f_name, b.l_name, b.email, b.mobile, c.name as contacttype, d.status as user_status, e.user_role    
			FROM tbl_users a, tbl_contact b, tbl_contacttype c, tbl_status d, tbl_user_role e
			WHERE  a.contact_id = b.id AND b.contacttype_id = c.id AND a.user_status = d.id AND a.user_role_id = e.id
				AND c.id <> 1 ORDER BY id;
