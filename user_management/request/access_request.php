...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <center><h3 class="m-0 font-weight-bold text-primary">New Access Request</h3></center>
  </div>
...
<div class="container">
  <div class="form-row">
    ...
    <div class="col-md-4 mb-3">
      ...
      <input type="date" class="form-control demoInputBox" id="doj" name= "doj" placeholder="Date of Joining" value = "<?= date('Y-m-d') ?>" required>
    </div>

    <div class="col-md-4 mb-3">
      <label for="validationDefault03" class="info">Exit date</label><span id="exit_date-info" class="info"></span>
      <input type="date" class="form-control demoInputBox" id="exit_date" name= "exit_date" placeholder="Exit Date" value="<?= date("Y-m-d", strtotime("+730 days", strtotime(date("Y-m-d")))); ?>">
    </div>
    ...
    <div class="col-md-4 mb-3">
      <label for="validationDefault03" class="info">Request Approver</label><span id="designation-info-info" class="info"></span>
        <select id="approver" name="approver" class="form-control demoInputBox" onchange="myLoadFunction()">
        <option value="-1">Select Approver</option>
            <?php
                $result = $gen->getAccessApproverList();
                if (!empty($result)) 
                {
                    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
                    {   
            ?> 
            <option 
                data-email= <?php echo $row['email']; ?> data-name= <?php echo $row['name']; ?>
                data-id= <?php echo $row['id']; ?>
                value=<?php echo $row['email']; ?>> <?php echo $row["name"]; ?></option>
            <?php   } 
                }
            ?>  
        </select>
        <input type="hidden" class="form-control demoInputBox" id="approver_id" name= "approver_id" value="<?php echo $row1["id"]; ?>" >
        <input type="hidden" class="form-control demoInputBox" id="approver_name" name= "approver_name" value="<?php echo $row1["name"]; ?>" >
        <input type="hidden" class="form-control demoInputBox" id="approver_email" name= "approver_email" value="<?php echo $row1["email"]; ?>" >
    </div>

    <script>
        function myLoadFunction()
        {
            var index = document.getElementById("approver").selectedIndex;
            var id = document.getElementById("approver").options[index].getAttribute("data-id");
            var name = document.getElementById("approver").options[index].getAttribute("data-name");
            var email = document.getElementById("approver").options[index].getAttribute("data-email");
    ...
    <div class="col-md-4 mb-3">
      <label for="validationDefault03" class="info">Request Message:</label><span id="pin-info" class="info"></span>
       <textarea " rows= 3 cols=5 maxlength="300" onKeyDown="return/[a-z0-9. ⌦←→⌫HT]/i.test(event.key)" class="form-control demoInputBox" id="message" name= "message" placeholder="Max 300 Characters" required> </textarea>
    </div>
