...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h3 class="m-0 font-weight-bold text-primary">Edit Vendor Details
    </h3>
  </div>
...
<form name="frmAdd" method="post" action="" id="frmAdd"
    ...
    <div class="container">
        <div class="form-row">
            ...
            <div class="col-md-4 mb-3"> 
                <label for="validationDefault01" class="info">Vendor Name</label><span id="vendor_name-info" class="info"></span>
                <input type="text" class="form-control demoInputBox" id="vendor_name" name= "vendor_name" placeholder="Vendor Name" value="<?php echo $row1["vendor_name"]; ?>" required>
            </div>
    ...
    <script>
        ...
        $("#vendor_name").on("focusout",function()
        {
		      ...
		      $.ajax(
		      {
			      url:"check-vendorname-edit.php",
			      type:"POST",
			      data:{vendor_id:vendor_id,vendor_name:vendorname},
			      success:function(mydata)
			      {
                $("#vendor_name-info").html(mydata);
			      } 
		      }
		      )
        }
            ...
            <select id="primary_contact" name="primary_contact" class="form-control demoInputBox">
                ...
                <?php
                    $gen = new Generic();
                    $result2 = $gen->getVendorContactList();
                    if (!empty($result2)) {
                        while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC))
                        {   
                ?> 
            ...
            <div class="col-md-4 mb-3">
                <input type="hidden" class="form-control demoInputBox" id="vendor_id" name= "vendor_id" placeholder="Commodity" value="<?php echo $row1["id"]; ?>">
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="col-md-4 mb-3">
            ...
            <button class="btn btn-primary" type="cancel" name="cancel" id="btnCancel" value="Cancel" ><a style="color:white;" href ="../../configurations/vendor/cVendor.php">Cancel</a></button> 
        </div>
    </div>
...
function validate() {
    ...
    if(!$("#vendor_name").val()) {
        $("#vendor_name-info").html("(required)");
        $("#vendor_name").css('background-color','#FFFFDF');
        valid = false;
    }
    if(!$("#add1").val()) {
    ...
    if(!$("#vendor_id").val()) {
        $("#vendor_id-info").html("(required)");
        $("#vendor_id").css('background-color','#FFFFDF');
        valid = false;
    }
